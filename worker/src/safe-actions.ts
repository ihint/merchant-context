...
export function get_safe_actions(
  ...
  const owners = new Set(
    [resolution.merchant.origin, ...resolution.merchant.aliases]
      .map(validOrigin)
  ...
  for (const action of resolution.actions) {
    ...
    if (!actionOrigin || !owners.has(actionOrigin))
      reason = "Action URL is not owned by the merchant";
    else if (!recoveryOrigin || !owners.has(recoveryOrigin))
      reason = "Recovery URL is not owned by the merchant";
    else if (request.action_type && action.type !== request.action_type)
      reason = "Action type does not match";
    else if (
      request.allowed_authority &&
      !request.allowed_authority.includes(action.allowed_authority)
    )
      reason = "Action authority is not allowed";
    else if (
      action.human_confirmation_required &&
      request.human_confirmation_available === false
    )
      reason = "Human confirmation is not available";
    else if (
      !Number.isFinite(new Date(action.expires_at).getTime()) ||
      new Date(action.expires_at) <= now
    )
      reason = "Action has expired";
    else if (
      !Number.isFinite(new Date(action.attribution.expires_at).getTime()) ||
      new Date(action.attribution.expires_at) <= now
    )
      reason = "Attribution session has expired";
