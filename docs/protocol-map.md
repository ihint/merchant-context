...
| Google and Gemini commerce | [Universal Commerce Protocol](https://ucp.dev/) | Discovery, catalog search and lookup, cart building, identity linking, checkout, order management, native checkout, embedded checkout, post-purchase capability exchange, AP2 payments, and REST or JSON-RPC transports | Automatic support outside participating platforms, agents, and businesses |
...
| Paid machine requests | HTTP 402 systems such as [x402](https://github.com/x402-foundation/x402) | Payment requirements, payment payloads, payment schemes, HTTP payment-required signaling, verification, settlement, discovery, and pay-per-request flows | Product fit, buyer consent, order recovery, or browser-native payment behavior |
...
- ACP uses date-based versions and lists `2026-04-17` as the latest stable OpenAPI, JSON Schema, examples, and changelog snapshot in the README.
  Source: https://github.com/agentic-commerce-protocol/agentic-commerce-protocol
  Checked: 2026-08-17.
- ACP exposes changelog entries through a `changelog/` directory and currently includes an `unreleased/` changelog directory.
...
- UCP states that core capabilities include catalog search and lookup, cart building, identity linking, checkout, and order management.
  Source: https://ucp.dev/
  Checked: 2026-08-17.
- UCP states that it is built on REST and JSON-RPC transports with AP2, A2A, and MCP support built in.
  Source: https://ucp.dev/
  Checked: 2026-08-17.
- UCP states that it is expanding to Lodging and Food, with detailed specifications coming soon.
  Source: https://ucp.dev/
  Checked: 2026-08-17.
...
- Schema.org `Offer` includes properties such as `availability`, `itemOffered`, and `hasMerchantReturnPolicy`.
  Source: https://schema.org/Offer
  Checked: 2026-08-17.
- Schema.org `Product` can carry offers and product identifiers used for merchant listings.
...
- Cloudflare remote MCP docs describe building and deploying remote MCP servers on Cloudflare.
  ...
  Checked: 2026-08-17.
- Cloudflare remote MCP docs state that remote MCP connections use Streamable HTTP and OAuth.
  Source: https://developers.cloudflare.com/agents/model-context-protocol/
  Checked: 2026-08-17.
...
- Cloudflare Web Bot Auth requires a key directory at `/.well-known/http-message-signatures-directory`.
  Source: https://developers.cloudflare.com/bots/reference/bot-verification/web-bot-auth/
  Checked: 2026-08-17.
- Cloudflare is experimenting with the `Forwarded` header for Web Bot Auth transitive trust.
  Source: https://developers.cloudflare.com/bots/reference/bot-verification/web-bot-auth/
  Checked: 2026-08-17.
...
- x402 describes itself as an open standard for internet-native payments across crypto and fiat forms of value.
  ...
  Checked: 2026-08-17.
- x402 v2 separates core types, payment logic, and transport representation so payment data can be carried over HTTP, MCP, A2A, or custom request-response transports.
  Source: https://raw.githubusercontent.com/x402-foundation/x402/main/specs/x402-specification-v2.md
  Checked: 2026-08-17.
- x402 v2 defines facilitator `/verify` and `/settle` APIs as HTTP REST endpoints.
  Source: https://raw.githubusercontent.com/x402-foundation/x402/main/specs/x402-specification-v2.md
  Checked: 2026-08-17.
- x402 v2 defines a discovery API for discoverable x402-enabled resources.
  Source: https://raw.githubusercontent.com/x402-foundation/x402/main/specs/x402-specification-v2.md
  Checked: 2026-08-17.
- MDN says HTTP `402 Payment Required` is nonstandard, reserved for future use, and handled by browsers as a generic 4xx status.
  ...
  Checked: 2026-08-17.
- RFC 9110 says HTTP `402 Payment Required` is reserved for future use.
